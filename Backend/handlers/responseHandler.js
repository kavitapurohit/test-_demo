...
let checktokenerror = (res, error) => {

    console.log('Authentication Error with Token --->', error);

    const response = {
        status: false,
        code: constant.HTTP_CODE.badRequest,
        message: error.details ? error.details[0].message : 'There is some issue with Authentication.',
        data: {}
    }
    _sendResponse(res, response);
}
